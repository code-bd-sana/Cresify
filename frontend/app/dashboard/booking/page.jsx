...
const Bookingpage = () => {
  ...
  const handleStatusUpdate = async (bookingId, newStatus) => {
    ...
    alert(
      t("booking.alerts.statusUpdated", {
  ...
  const getPaymentBadge = (status) => {
    ...
    const statusText = status
      ...
      : t("booking.paymentStatus.unknown");
      ...
      <div className='min-h-screen bg-gray-50 p-6'>
        <div className=' mx-auto'>
          <div className='bg-red-50 border border-red-200 rounded-lg p-4'>
            <p className='text-red-700'>{t("booking.errors.loadingError")}</p>
            <button
              onClick={() => refetch()}
              ...
              <div>
                ...
                <p className='text-2xl font-bold text-purple-600'>
                  ...
                  {bookings
                    .reduce(
                      (sum, booking) =>
                        sum + (booking.provider?.hourlyRate || 0),
                      0
                    )
                    .toFixed(2)}
                </p>
              ...
              <div className='flex items-center gap-2'>
                ...
                <select
                  ...
                  <option value='pending'>
                    {t("booking.filters.pending")}
                  </option>
                  <option value='accept'>{t("booking.filters.accept")}</option>
                  <option value='reject'>{t("booking.filters.reject")}</option>
                  <option value='completed'>
                    {t("booking.filters.completed")}
                  </option>
                  <option value='cancelled'>
                    {t("booking.filters.cancelled")}
                  </option>
                </select>
              </div>
              ...
              <thead className='bg-gray-50'>
                <tr>
                  <th className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                    {t("booking.table.bookingId")}
                  </th>
                  <th className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                    {t("booking.table.provider")}
                  </th>
                  <th className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                    {t("booking.table.customer")}
                  </th>
                  <th className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                    {t("booking.table.dateTime")}
                  </th>
                  <th className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                    ...
                    <td
                      ...
                      <div className='flex flex-col items-center justify-center'>
                        ...
                        <p className='text-lg font-medium text-gray-900'>
                          {t("booking.noBookings.title")}
                        </p>
                        <p className='text-gray-600 mt-1'>
                          {bookings.length === 0
                            ? t("booking.noBookings.empty")
                            : t("booking.noBookings.filter")}
                        </p>
                      </div>
                    </td>
                  </tr>
                    ...
                    <tr key={booking._id} className='hover:bg-gray-50'>
                      <td className='px-6 py-4 whitespace-nowrap'>
                        <div className='text-sm font-medium text-gray-900'>
                          #{booking._id?.slice(-8) || t("common.notAvailable")}
                        </div>
                      ...
                      <td className='px-6 py-4'>
                        <div className='text-sm text-gray-900 font-medium'>
                          {booking.customer?.email || t("common.notAvailable")}
                        </div>
                        <div className='text-xs text-gray-500'>
                          {booking.customer?.fullName ||
                            t("common.notAvailable")}
                      ...
                      <td className='px-6 py-4 whitespace-nowrap'>
                        <div className='flex items-center gap-2'>
                          ...
                          <button
                            ...
                            title={t("booking.actions.viewDetails")}>
                          ...
                          <div className='relative'>
                            <select
                              ...
                              <option value='' disabled>
                                {t("booking.actions.updateStatus")}
                              </option>
                              <option value='pending'>
                                {t("booking.actions.setToPending")}
                              </option>
                              <option value='accept'>
                                {t("booking.actions.setToAccepted")}
                              </option>
                              <option value='reject'>
                                {t("booking.actions.setToRejected")}
                              </option>
                              <option value='completed'>
                                {t("booking.actions.setToCompleted")}
                              </option>
                              <option value='cancelled'>
                                {t("booking.actions.setToCancelled")}
                              </option>
                            </select>
              ...
              <div className='flex flex-col md:flex-row items-center justify-between gap-4'>
                <div className='text-sm text-gray-700'>
                  {t("booking.pagination.showing", {
                    ...
                    end: Math.min(currentPage * limit, totalBookings),
                    total: totalBookings,
                  })}
                </div>
          ...
          <div className='mt-6 grid grid-cols-1 md:grid-cols-3 gap-4'>
            <div className='bg-white rounded-lg shadow p-4'>
              <h3 className='text-sm font-medium text-gray-900 mb-2'>
                {t("booking.summary.statusDistribution")}
              </h3>
              <div className='space-y-2'>
                {["pending", "accept", "reject", "completed", "cancelled"].map(
                  (status) => {
            ...
            <div className='bg-white rounded-lg shadow p-4'>
              <h3 className='text-sm font-medium text-gray-900 mb-2'>
                {t("booking.summary.paymentStatus")}
              </h3>
              <div className='space-y-2'>
                {["processing", "completed"].map((status) => {
                  const count = bookings.filter(
                    (b) => b.paymentStatus === status
                  ).length;
                  const percentage =
                    bookings.length > 0 ? (count / bookings.length) * 100 : 0;
            ...
            <div className='bg-white rounded-lg shadow p-4'>
              <h3 className='text-sm font-medium text-gray-900 mb-2'>
                {t("booking.summary.recentActivity")}
              </h3>
